...
class DDBMurd(Murd):
    ...
    def read(self,
        ...
        if type(row) is list:
            ...
            kce = Key("ROW").eq(str(row))
            if col is not None:
                kce = kce & Key("COL").begins_with(str(col))

            elif greater_than_col is not None and less_than_col is not None:
                kce = kce & Key("COL").between(str(greater_than_col), str(less_than_col))

            elif greater_than_col is not None:
                kce = kce & Key("COL").gt(str(greater_than_col))
            elif less_than_col is not None:
                kce = kce & Key("COL").lt(str(less_than_col))
